...
def apply_bald_hairstyle(input_path, output_path, color_correction = 0):
    '''
    applies the bald hairstyle to the image
    '''
    anchor_points = findingAnchorPoints(input_path)[1:6]      #left forehead, right forehead, left ear, right ear
    hair_anchor = [(38, 105), (145, 105), (20, 161), (167, 161)]
    apply_hair_color(input_path, output_path, 'Hairstyles/bald.png', anchor_points, hair_anchor)

def apply_curly_hairstyle(input_path, output_path, color_correction = 0):
    '''
...
def apply_mullet_hairstyle(input_path, output_path, color_correction = 0):
    ...
    apply_hair(input_path, output_path, 'Hairstyles/mullet.png', anchor_points, hair_anchor)

def apply_waves_hairstyle(input_path, output_path, color_correction = 0):
    '''
    applies the waves hairstyle to the image
    '''
    anchor_points = findingAnchorPoints(input_path)[2:6]      #left forehead, right forehead, left ear, right ear
    hair_anchor = [(50, 140), (185, 140), (30, 195), (205, 195)]
    apply_hair(input_path, output_path, 'Hairstyles/waves.png', anchor_points, hair_anchor)


def apply_hair(input_path, output_path, overlay_path, face_anchor, hair_anchor):
    '''
    ...
    background = cv2.imread(input_path, cv2.IMREAD_COLOR)
...
def apply_hair_color(input_path, output_path, overlay_path, face_anchor, hair_anchor):
    '''
